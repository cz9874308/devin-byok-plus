...
代理侧（ESM，位于 `src/proxy/logging/`）：

| 文件 | 职责 |
| --- | --- |
| `file-logger.js` | 核心。解析日志配置、拼接路径、异步批量追写 JSONL、按大小轮转、启动时清理过期文件。对外仅暴露 `logEvent(event)` / `logBlob(turnId, kind, content)` / `isVerbose()`。**不 import 任何业务模块**，避免循环依赖（`ws-bridge.js`、`chat.js` 都要引它）。 |
| `anomalies.js` | 异常代码常量表 + `severity` 映射。纯数据 + 纯函数，可单测。 |
| `request-context.js` | 单请求上下文。`createRequestLog(meta)` 返回句柄，收集本轮字段与 anomaly 列表，流结束时落一条 `chat_turn` 汇总，并逐条落 `anomaly` 事件。 |

扩展侧（CommonJS）：

| 文件 | 职责 |
| --- | --- |
| `src/managers/log-writer.js` | 精简写入器（约 60 行），只做追写 + 轮转。因为扩展侧是 CJS、代理侧是 ESM，两边无法共用同一模块文件。 |

### 3.2 改造点（最小侵入）

- `src/proxy/hybrid-server.js` / `src/proxy/inference-proxy.js`：启动时初始化 logger（读 env、建目录、清理过期文件、写一条 `proc` 启动事件）。
- `src/proxy/handlers/chat.js`：在 `handleGetChatMessage` 创建 request context；在既有的断流/超时/重试/强制 stop 分支旁挂 `ctx.anomaly(...)`（`chat.js:1100-1110`、`1159-1194`、`1582-1616`、`1619-1651`、`1715-1729`）。
- `src/proxy/handlers/openai-stream.js` / `anthropic-stream.js`：在既有降级/兜底分支挂 `anomaly`（`openai-stream.js:203-249`、`anthropic-stream.js:202-228`）。两个 processor 目前不持有 request context，通过 `setRequestLog(ctx)` 注入，沿用它们已有的 `setAllowedTools` / `setSoundEligible` 注入风格。
...
**`proc`** — 进程生命周期：启动、端口绑定、退出码、`uncaughtException`。扩展侧的启停 / 配置热更 / 端口回退也用此类型，`proc:"ext"`。
...
4 个配置项，全部加入 `writeEnvConfig` 白名单（`proxyManager.js:523`）与 `buildRuntimeConfigPatch`（`proxyManager.js:587`）。

| key | 默认值 | 说明 |
| --- | --- | --- |
| `FILE_LOG_ENABLED` | `true` | 总开关。摘要级日志每轮仅 1–10 行 JSON，常开才能捕获难复现问题 |
| `FILE_LOG_VERBOSE` | `false` | 原文 blob 开关 |
| `FILE_LOG_MAX_MB` | `10` | 单文件轮转阈值 |
| `FILE_LOG_RETAIN_DAYS` | `7` | 保留天数 |

进入白名单的原因：不进白名单的 key 会被 `writeEnvConfig` 走 `tmp4` 原样透传（`proxyManager.js:524`、`581-583`），位置与注释不受管理。进入白名单后侧栏可管理、有固定位置。

加入 `buildRuntimeConfigPatch` 后，开关可经 `/api/config` **热更新、无需重启代理**——`FILE_LOG_VERBOSE` 尤其需要，便于复现问题时即时开启。
...
~/.devin-byok-plus/logs/
  ...
  proxy-2026-07-29.1.jsonl      # 超过 MAX_MB 后的轮转序号
...
**清理**：仅在**进程启动时**扫描目录一次，删除超过 `RETAIN_DAYS` 的文件。不使用定时器，避免长期运行进程中多一个无必要的 timer。

## 9. 错误处理

- 日志子系统自身的任何 IO 错误一律吞掉（目录不可写、磁盘满等），绝不向业务路径抛出或打断 agent 交互。
- 首次写入失败时向 stdout 打印一次告警（经 `proxyManager.log()` 出现在输出面板），之后静默，避免刷屏。
- `FILE_LOG_ENABLED=false` 时所有接口为空实现，零 IO、零队列分配。

## 10. 测试

沿用现有 21 个单测的 `.mjs` + node 内建 runner 风格，置于 `test/unit/`。
...
- `chat_turn` 只落一次（幂等，对应现有 `streamFinished` 那类双重 finalize 风险）
...
| verbose 忘关导致磁盘占用 | 默认关闭 + 按天轮转 + `RETAIN_DAYS` 启动清理 |
| 日志代码引入新崩溃点 | 自身 IO 错误全吞；不 import 业务模块避免循环依赖 |
