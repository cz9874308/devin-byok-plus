...
- OPUS_BYOK 条目 `field1`（模型数值 ID）= 277（已确认）。其余三个模型 ID 待实现阶段抓包补齐。
...
`SLOT_MODEL_ID` 中除 OPUS=277 外，其余三个模型（OPUS_THINKING/SONNET/SONNET_THINKING）的 field1 数值 ID 需抓一次 `GetUserStatus` 补齐。这是实现的第一步，用临时解析脚本读取（用完即删）。补齐前可先只登记 OPUS 使 BYOK1 可用。
